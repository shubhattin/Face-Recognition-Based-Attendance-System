from flask import Flask, render_template, request, redirect, url_for, flash, Response  # pyright: ignore[reportMissingImports]
import sqlite3
...
def index():
    ...
    return render_template('index.html', selected_date='', no_data=False,
                           has_images=has_images, has_features=has_features, people=people)
...
def start_attendance():
    try:
        ...
        if not has_images:
            flash("No face images found. Please open Face Registration and save several images first.")
            return redirect(url_for('index'))
