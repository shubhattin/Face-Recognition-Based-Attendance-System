...
def return_128d_features(path_img):
    ...
    if img_bgr is None:
        logging.warning("failed to read image: %s", path_img)
        return 0
...
def return_features_mean_personX(path_face_personX):
    ...
    if photos_list:
        for i in range(len(photos_list)):
            ...
            logging.info(
                "%-40s %-20s",
                " / Reading image:",
                path_face_personX + "/" + photos_list[i],
            )
    ...
    if features_list_personX:
        features_mean_personX = np.array(features_list_personX, dtype=float).mean(
            axis=0
        )
    else:
        features_mean_personX = np.zeros(128, dtype=float, order="C")
...
def main():
    ...
    with open("data/features_all.csv", "w", newline="") as csvfile:
        ...
        for person in person_list:
            ...
            # Skip writing if features are all zeros (no valid face found)
            if float(np.linalg.norm(features_mean_personX)) < 1e-8:
                logging.warning("Skip %s: no valid face features detected", person)
                continue
            ...
            logging.info("\n")
        logging.info(
            "Save all the features of faces registered into: data/features_all.csv"
        )
