...
class Face_Register:
    ...
    def GUI_info(self):
        tk.Label(
            self.frame_right_info, text="Face register", font=self.font_title
        ).grid(row=0, column=0, columnspan=3, sticky=tk.W, padx=2, pady=20)
        ...
        tk.Label(self.frame_right_info, text="Faces in current frame: ").grid(
        ...
        ).grid(row=5, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        tk.Button(
            self.frame_right_info, text="Clear", command=self.GUI_clear_data
        ).grid(row=6, column=0, columnspan=3, sticky=tk.W, padx=5, pady=2)

        # Step 2: Input name and create folders for face
        tk.Label(
            self.frame_right_info, font=self.font_step_title, text="Step 2: Input name"
        ).grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        tk.Button(
            self.frame_right_info, text="Input", command=self.GUI_get_input_name
        ...
        ).grid(row=9, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)

        tk.Button(
            self.frame_right_info,
            text="Save current face",
            command=self.save_current_face,
        ).grid(row=10, column=0, columnspan=3, sticky=tk.W)
    ...
    def create_face_folder(self):
        ...
        if self.input_name_char:
            ...
            self.current_face_dir = (
                self.path_photos_from_camera + "person_" + str(self.existing_faces_cnt)
            )
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    self.face_ROI_image = np.zeros(
                        (int(self.face_ROI_height * 2), self.face_ROI_width * 2, 3),
                        np.uint8,
                    )
    ...
    def get_frame(self):
        try:
            ...
            print(f"Error: No video input!!! {e}")
            return False, None
        ...
        if ret and self.current_frame is not None:
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    self.current_frame = cv2.rectangle(
                        self.current_frame,
