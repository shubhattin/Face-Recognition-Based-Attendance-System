...
# NOTE: dlib is a C-extension module, so static analyzers like Pylint
# sometimes cannot see its dynamically provided members. We explicitly
# disable the false-positive `no-member` warnings on these known APIs.
...
class Face_Recognizer:
    def __init__(self):
        ...
        # Smooth recognition and avoid flapping: require N consecutive hits
        self.required_consecutive_hits = 3
        self.recognized_consecutive_counts = {}
        # Cooldown to avoid re-processing same person repeatedly within session (seconds)
        self.cooldown_seconds = 60
        self.last_mark_time_by_name = {}
    ...
    def centroid_tracker(self):
        for i in range(len(self.current_frame_face_centroid_list)):
            ...
            self.current_frame_face_name_list[i] = self.last_frame_face_name_list[
                last_frame_num
            ]
    ...
    def draw_note(self, img_rd):
        ...
        cv2.putText(
            img_rd,
            "Face Recognizer with Deep Learning",
            (20, 40),
            self.font,
            1,
            (255, 255, 255),
        ...
        cv2.putText(
            img_rd,
            "Faces:  " + str(self.current_frame_face_cnt),
            (20, 160),
            self.font,
            0.8,
            (0, 255, 0),
            ...
            while stream.isOpened():
                ...
                # 6.2  If cnt of faces changes, 0->1 or 1->0 or ...
                    ...
                    # 6.2.2 / Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                        ...
                        self.current_frame_face_name_list = []
                        # Convert to RGB for descriptor computation
                        ...
                        for k in range(len(faces)):
                            ...
                            for i in range(len(self.face_features_known_list)):
                                # Skip invalid/zero feature rows
                                try:
                                    feat_known = np.array(
                                        self.face_features_known_list[i], dtype=float
                                    )
                                except Exception:
                                    ...
                                    continue
                                if np.linalg.norm(feat_known) < 1e-6:
                                    self.current_frame_face_X_e_distance_list.append(
                                        999999999
                                    )
                                    continue
                                e_distance_tmp = self.return_euclidean_distance(
                                    self.current_frame_face_feature_list[k], feat_known
                                )
                                logging.debug(
                                    "      with person %d, the e-distance: %f",
                                    i + 1,
                                    e_distance_tmp,
                                )
                                self.current_frame_face_X_e_distance_list.append(
                            ...
                            # Margin check between best and second-best matches
                            sorted_dists = sorted(
                                self.current_frame_face_X_e_distance_list
                            )
                            min_dist = sorted_dists[0]
                            second_dist = (
                                sorted_dists[1] if len(sorted_dists) > 1 else 999999999
                            )
                            threshold = 0.45
                            margin = 0.03
                            if (
                                min_dist < threshold
                                and (second_dist - min_dist) >= margin
                            ):
                                name_rec = self.face_name_known_list[similar_person_num]
                                self.current_frame_face_name_list[k] = name_rec
                                logging.debug("  Face recognition result: %s", name_rec)

                                # Consecutive confirmation to stabilize
                                cnt = (
                                    self.recognized_consecutive_counts.get(name_rec, 0)
                                    + 1
                                )
                                self.recognized_consecutive_counts[name_rec] = cnt

                                # Cooldown check
                                now_ts = time.time()
                                last_ts = self.last_mark_time_by_name.get(name_rec, 0)

                                if (
                                    cnt >= self.required_consecutive_hits
                                    and (now_ts - last_ts) >= self.cooldown_seconds
                                ):
                                    self.attendance(name_rec)
                                    self.last_mark_time_by_name[name_rec] = now_ts
                            else:
                                logging.debug(
